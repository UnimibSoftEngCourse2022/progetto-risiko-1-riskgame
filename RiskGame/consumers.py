...
class PartitaConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        if (tipo == 'messaggio'):
            ...
            Partita.disconnettiGiocatore(
                text_data_json['idPartita'], self.scope['user'].username)
    ...
    def evento_gioco(self, event):
        ...
        # Dice agli utenti di togliere la chat e disegnare il gioco
        if (tipo == 'rappresentazionePartita'):
            self.send(text_data=json.dumps({
                'tipo': tipo,
                'sender': mittente,
    ...
    def assegnazioneTerritoriTruppeIniziali():
        ...
        for i in xlistaGiocatori:
            ...
            for j in range(k, h):
                listaTerritori.append(ClasseTerritorio(
                    0, i, j.NomeTerritorio, j.Continente))
    ...
    def chiamataMetodoAssegnazioneTruppeTerritorio(classeGiocatore):
        ...
        for i in listaTerritori:
            if listaTerritori.giocatore == classeGiocatore.nickname:
                xlistaTerritori.append(ClasseTerritorio(
                    listaTerritori.numTruppe, listaTerritori.giocatore, listaTerritori.nome, listaTerritori.continente))
    ...
    def inizializzaPartita(self):
        ...
        for giocatore in listaTemp:
            listaGiocatori.append(dataclasses.asdict(
                ClasseGiocatore(giocatore, 0, [])))
